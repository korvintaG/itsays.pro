...
import { type UserResponseToken } from "../../features/auth/user-types";
...
export class Api {
  ...
  async request<T>(endpoint: string, options: RequestInit) {
    try {
      ...
      window.console.log('API Request:', {
        url: fullUrl,
      ...
      window.console.log('API Response:', {
        url: fullUrl,
        status: res.status,
      ...
      const fullUrl = `${this.baseUrl}${endpoint}`;
      window.console.error('API Request Error:', {
        url: fullUrl,
        error: error,
      ...
      if (!fetchError.statusCode && fetchError.message.includes('Failed to fetch')) {
        ...
        const isHttpApi = this.baseUrl.startsWith('http://');
        let errorMsg = 'Не удалось подключиться к серверу. ';
        
        if (isHttpsPage && isHttpApi) {
          errorMsg += 'Проблема: HTTPS страница не может делать запросы к HTTP серверу (mixed content). ';
          errorMsg += `Попробуйте использовать HTTPS для API или запустить frontend на HTTP.`;
        } else {
          errorMsg += 'Проверьте:\n- Доступен ли сервер по адресу ' + this.baseUrl + '\n- Настройки CORS на сервере\n- Интернет-соединение';
        }
        
        fetchError.message = errorMsg;
      }
