import { RequestStatus, type IDetailsAddHookRes, type IDetailsEditHookRes } from "../types/types-for-hooks";
import { type User, Role } from "../../features/auth/user-types";
//import { EditAccessStatus } from "./utils";

import clsx from "clsx";
import { type ObjectCreationWithModeration, type SimpleNameObject, VerificationStatus } from "../types/entity-types";
...
export function genHeaderText(
  ...
  if (!id) return `Добавление ` + text;  // ${newWord} 
  else if (readOnly) return `Просмотр ${text}`;
  else return `Изменение ${text}`;  //  [${name}]
}
